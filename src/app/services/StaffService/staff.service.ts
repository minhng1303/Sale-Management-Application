...
export class StaffService implements OnInit {
  ...
  constructor(private db: AngularFirestore, private firebase: FirebaseService) {
    this.staffs = this.db.collection<Staff>('staff').valueChanges();
  }
    ...
    await this.db
      ...
      .then((data) =>
        ...
        data.docs.forEach((item) => {
          let ele = item.data();
          if (e.email === ele['email']) {
            this.db.collection('staff').doc(item.id).delete();
